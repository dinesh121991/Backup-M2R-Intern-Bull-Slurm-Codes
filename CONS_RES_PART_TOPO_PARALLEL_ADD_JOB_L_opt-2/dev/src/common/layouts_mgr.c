...
int _layouts_entity_set_kv_uint32(layout_t* l, entity_t* e, char* key, void* value)
{
	...
	_normalize_keydef_key(key_keydef, PATHLEN, key, l->type);
	return entity_set_data(e, key_keydef, value, sizeof(uint32_t));
}
...
char* layouts_entity_get_parent_name(char* l, char* e)
{
	...
	entity_node_t *node_1 = entity_get_node (entity_1,layout_1);
	xtree_node_t *node_2 = xtree_get_parent (tree_1,(xtree_node_t *)(node_1->node));

	entity_node_t *enode = (entity_node_t*) xtree_node_get_data(node_2);
	return entity_get_name (enode->entity);
}

int layouts_multi_entity_set_kv(char* l,List entity,
...
int layouts_multi_entity_get_kv(char* l, List entity, char* key, void *value,
                                int count, size_t length)
{	
	char *entity_name;
	ListIterator li = list_iterator_create(entity);
	int i=0,rc;
	
	while ((entity_name = list_next(li) ) != NULL && i < count)
	{				
		//info("Layout_mgr entity:%s",entity_name);
		 _layouts_multi_entity_wrapper(_layouts_multi_entity_get_kv, l, entity_name,
				key, (value+i), length);
		if(rc == SLURM_ERROR)
		{
			info("rc is 0");
