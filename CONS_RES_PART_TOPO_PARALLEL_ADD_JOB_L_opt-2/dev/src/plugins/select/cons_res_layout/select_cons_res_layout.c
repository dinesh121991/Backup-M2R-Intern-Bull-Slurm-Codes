...
void* _add_job_serial_multiset(void *job_rec)
{
	...
	int core_count=0,socket_count=0;
	int core_size;
		
	if(core_end >= 0)
	{
		core_size = (core_end)+1;
	}
	else
	{
		core_size = core_end;
	...
	int str_size;
	char *core_name,*socket_name,*pre_socket_name="-1";
	char *core_base_name = "core";
	
	List core_list = list_create(slurm_destroy_char);
	
	List socket_list = list_create(slurm_destroy_char);
	
	for( i=core_begin ; i < core_size ; i++ )
	{
		...
		{
			//info ("Enter - %d",i);
			core_name = xmalloc(15* sizeof(char));
			sprintf(core_name, "%s%d",core_base_name,i);

			if (list_append(core_list,core_name) != NULL )
			{
				core_count++;
				socket_name = layouts_entity_get_parent_name (L_CONS_RES_PARTITION,core_name);
				if(socket_name != NULL)
				{
					...
					{									
						...
						list_append(socket_list,(void *)socket_name);
						socket_count++;
						pre_socket_name = socket_name;
					}
				}
		...
		/*info("core_list[%d]:%s socket_list[%d]:%s",core_count-1,core_list[core_count -1],
		     socket_count-1,socket_list[socket_count -1]);*/
	}
	(core_count)++;
	(socket_count)++;
	info("core_count:%d,socket_count :%d",core_count,socket_count);
	/*ListIterator li = list_iterator_create(core_list);
	while((core_name =list_next(li) ) != NULL )
	{
		info("list: core_name:%s ",core_name);
		
	}
	li = list_iterator_create(socket_list);
	while((socket_name = list_next(li)) != NULL )
	{
		info("list: socket_name:%s ", socket_name);
		
	}*/
	
	info("After bitmap2core_socket_list");
	info("Before multi set");
	uint32_t set=1;
	layouts_multi_entity_set_kv (L_CONS_RES_PARTITION,core_list,L_ALLOCATED_CORE_COUNT,&set,sizeof(set));
	info("After multi entity set");
	layouts_multi_entity_set_kv (L_CONS_RES_PARTITION,socket_list,L_ALLOCATED_SOCKET_COUNT,&set,sizeof(set));
	info("After multi entity set-2");
...
void* _rm_job_serial_multiset(void *job_rec)
{
	...
	info("before _rm_job_serial_multiset, bitmap2core_socket_list"); 
	int core_count=0,socket_count=0;
	...
	if(layouts_entity_pull_kv (L_CONS_RES_PARTITION,L_CLUSTER,L_ALLOCATED_SUM_CORES_CLUSTER)==SLURM_SUCCESS)
	{
		if(layouts_entity_pull_kv (L_CONS_RES_PARTITION,L_CLUSTER,L_ALLOCATED_SUM_SOCKETS_CLUSTER)==SLURM_SUCCESS)
		{
			if(layouts_entity_pull_kv (L_CONS_RES_PARTITION,L_CLUSTER,L_ALLOCATED_SUM_NODES_CLUSTER)==SLURM_SUCCESS )
			{
				info("L-Job_res after add pull Cluster level allocated_sum_cores:%d allocated_sum_sockets:%d allocated_sum_nodes:%d",
	...
	info("_rm_job_serial_multiset return:%d",slurm_status);
	int tp_index =1 ;
	pthread_exit((void*) tp_index);
}
...
extern int select_p_node_init(struct node_record *node_ptr, int node_cnt)
{
	...
	for( i=0 ; i < switch_record_cnt ; i++ )
	{
		...
		{
			level_switch_index[switch_record_table[i].level]
				[level_switch_count_tmp[switch_record_table[i].level]] = i;
			/*info("switch_record_table[i].level:%d level_switch_index[switch_record_table[i].level][level_switch_count_tmp[switch_record_table[i].level]]:%d ",
		     switch_record_table[i].level,level_switch_index[switch_record_table[i].level]
				[level_switch_count_tmp[switch_record_table[i].level]]);*/
		}
		else
		{
...
// Core,Socket Bitmap position Correlation with Layout
	...
	List core_list = list_create(list_destroy);
	list_append(core_list,"core0") ;
	list_append(core_list,"core1000") ;
	list_append(core_list,"core100") ;
	list_append(core_list,"core3800") ;
