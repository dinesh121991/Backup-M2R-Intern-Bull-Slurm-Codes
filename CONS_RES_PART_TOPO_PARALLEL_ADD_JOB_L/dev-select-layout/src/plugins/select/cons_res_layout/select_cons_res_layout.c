...
int _add_job_ll(struct job_resources *job, uint32_t job_id)
{
	...
	{
		if(layouts_entity_pullget_kv (L_CONS_RES_PARTITION,L_CLUSTER,L_ALLOCATED_SUM_SOCKETS_CLUSTER,&allocated_sum_sockets,L_T_UINT32)==SLURM_SUCCESS)
		{
...
static int _add_job_to_res(struct job_record *job_ptr, int action)
{
	...
	List gres_list;
	int i, n,rc,status,start,end,node_begin,node_end,node_count;
	bitstr_t *core_bitmap;
	char str_i[40];

	bool serial = false ;

	pthread_t threads_add[NUM_THREADS];
	pthread_attr_t attr;
	...
	node_begin = bit_ffs(job->node_bitmap);
	node_end = bit_fls(job->node_bitmap);
	node_count = node_end - node_begin;
	...
	bit_fmt(str_i,sizeof(str_i),job->core_bitmap_original_job);
	info("core_bitmap_original_job:%s nb_cores_cluster:%d nb_nodes:%d",str_i,size,nodes);
	
	if( node_count > 0 )
	{
		if( node_count <= NUM_THREADS || node_count <= NUM_THREADS*4)
		{
			serial = true ;
			info("before add_job_l");
			_add_job_l (job,job_ptr->job_id);
			info("After add_job_l");
		}
		else
		{
			int end_offset;
			...
			end = start = node_begin;
			end_offset= node_count / NUM_THREADS;
			...
			for( i=0; i < NUM_THREADS ; i++)
			{
				...
				{
					tps[i].end = end + end_offset;
				}
		
				info("Main: creating thread %d\n", i);
				rc = pthread_create(&threads_add[i], &attr,_add_job_parallel, (void *)i); 
		  		if (rc) 
				{
	...
	//info("cons_res: _add_job_to_res: job %u act %d ", job_ptr->job_id,action);
	...
	if (action != 1) {
		...
		for (i = 0; i < p_ptr->num_rows; i++) {
			if (!_can_job_fit_in_row(job, &(p_ptr->row[i])))
				continue;
			//info("L-cons_res: adding job %u to part %s row %u",job_ptr->job_id, p_ptr->part_ptr->name, i);
	...
	info("Reached");
	if( serial == false && (node_count > NUM_THREADS || node_count > NUM_THREADS*4) )
	{
		info("Reached");
		for( i=0 ; i< NUM_THREADS ; i++ )
		{
			rc = pthread_join(threads_add[i], &status);
			if (rc) 
			{
