...
uint16_t *cr_node_num_sockets = NULL;
uint32_t *cr_node_sockets_offset = NULL;
				     ...
				     uint16_t fast_schedule)
{
	...
	for (n = 0; n < node_cnt; n++) {
		...
		if (fast_schedule) {
			...
			sockets = node_ptr[n].config_ptr->sockets;
		} else {
			cores  = node_ptr[n].cores;
			cores *= node_ptr[n].sockets;
			sockets = node_ptr[n].sockets;
		}
#endif
		cr_node_num_cores[n] = cores;
		cr_node_num_sockets[n] = sockets;
		if (n > 0) {
			cr_node_cores_offset[n] = cr_node_cores_offset[n-1] +
						  cr_node_num_cores[n-1] ;
			cr_node_sockets_offset[n] = cr_node_sockets_offset[n-1] +
				cr_node_num_sockets[n-1] ;
		} 
		else
		{
	...
	cr_node_sockets_offset[node_cnt] = cr_node_sockets_offset[node_cnt-1] + 
		cr_node_num_sockets[node_cnt-1] ;
...
extern void cr_fini_global_core_data(void)
{
	...
	xfree(cr_node_num_sockets);
	xfree(cr_node_sockets_offset);
}
