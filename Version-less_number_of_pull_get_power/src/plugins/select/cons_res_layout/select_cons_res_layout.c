...
#define L_CONS_RES_TOPOLOGY   "cons_res_topology"
...
#define L_ALLOCATED_THREADS_PER_CORE "AllocatedThreadsPerCore"

// For Threads level information in CONS_RES_PARTITION
#define L_NUM_SUM_THREADS_CLUSTER "NumSumThreadsInCluster"
#define L_NUM_SUM_THREADS_NODE "NumSumThreadsInNode"
#define L_NUM_SUM_THREADS_SOCKET "NumSumThreads"

#define L_ALLOCATED_SUM_THREADS_CLUSTER "AllocatedSumThreadsInCluster"
#define L_ALLOCATED_SUM_THREADS_NODE "AllocatedSumThreadsInNode"
#define L_ALLOCATED_SUM_THREADS_SOCKET "AllocatedSumThreads"

#define L_SELECTED_SUM_THREADS_CLUSTER "SelectedSumThreadsInCluster"
#define L_SELECTED_SUM_THREADS_NODE "SelectedSumThreadsInNode"
#define L_SELECTED_SUM_THREADS_SOCKET "SelectedSumThreads"
...
#define L_SELECTED_SUM_NODES_IN_LEVEL  "AvailableSumNodesInLevel" // Network
...
#define L_SELECTED_SUM_CPUS_IN_LEVEL "AvailableSumCpusInLevel"  // Network
//#define L_SELECTED_SUM_NODES_IN_LEVEL "SelectedSumNodesInLevel"
#define L_SELECTED_CPU_COUNT_IN_NODE "AvailableCpusCount" // Network
#define L_SELECTED_NODE_COUNT "AvailableNodeCount" //Network

#define L_SWITCHES_REQUIRED_IN_LEVEL "SwitchesRequiredInLevel" //Network
#define L_SWITCHES_REQUIRED_IN_NODE "SwitchesRequiredCount" //Network
#define L_SWITCHES_TO_PROCESS_IN_LEVEL "SwitchToProcessInLevel" //Network
...
int root_switch_index __attribute__((weak_import)) ;
int leaf_switch_cnt __attribute__((weak_import));
int *leaf_switch_index __attribute__((weak_import));
int **level_switch_index __attribute__((weak_import));
int *level_switch_count __attribute__((weak_import));
int nb_cores_cluster __attribute__((weak_import));
#else
...
uint32_t num_sum_nodes=0,free_sum_nodes=0,allocated_sum_nodes=0,num_sum_cores=0,free_sum_cores=0,allocated_sum_cores=0,
		num_sum_sockets=0,free_sum_sockets=0,allocated_sum_sockets=0,num_sum_threads=0,free_sum_threads=0,allocated_sum_threads=0;

#define NUM_THREADS 2 // xeon based server will have 16 processors so we are using atleast 2, but define later by processor configuration

//int nb_cores_cluster;
//bitstr_t *core_bitmap_original_job;

int switch_levels_max;
int root_switch_index;
int leaf_switch_cnt;
int *leaf_switch_index;
Buf *cons_res_topology_buf;
...
threads_parameters tps[NUM_THREADS];
...
int bitmap2core_socket_list(bitstr_t *core_map,char **core_list,char **socket_list,
                            int *core_count,int *socket_count)
{
	int core_size = bit_fls(core_map);
	core_size = (core_size - bit_ffs(core_map))+1;
	int i,j;
	int str_size;
	char core_name[30],*socket_name,*pre_socket_name="-1";
	char *core_base_name = "core";
	core_list = xmalloc(core_size * sizeof(char *));
	socket_list = xmalloc(core_size * sizeof(char *));
	for( i=0 ; i < core_size ; i++ )
	{
		if( !bit_test(core_map,i) )
			continue;
		else
		{
			//info ("Enter - %d",i);
			sprintf(core_name, "%s%d",core_base_name,i);			
			str_size = strlen(core_name);
			core_list[*core_count] = xmalloc(str_size * sizeof(char));
 			strcpy(core_list[*core_count],core_name);
			(*core_count)++;
			socket_name = layouts_entity_get_parent_name (L_CONS_RES_PARTITION,core_name);
			if(socket_name == NULL)
			{
				return 1;
			}
			else
			{
				...
				if( strcmp(socket_name,pre_socket_name) == 0)
				{
					continue;
				}
				else
				{					
					//j= *socket_count;
					str_size = strlen(socket_name);
					socket_list[*socket_count] = xmalloc(str_size * sizeof(char));
					strcpy(socket_list[*socket_count] ,socket_name);
					(*socket_count)++;
					pre_socket_name = socket_name;
				}
			}
		}
		/*info("core_list[%d]:%s socket_list[%d]:%s",*core_count,core_list[*core_count],
		     *socket_count,socket_list[*socket_count]);*/
	}
	(*core_count)++;
	(*socket_count)++;
	info("core_list[%d] socket_list[%d]",*core_count,*socket_count);
}

hostlist_t _bitmap2hostlist (bitstr_t *bitmap) 
{
	int i, first, last;
	hostlist_t hl;
...
hostlist_t _bitmap2core_hostlist (bitstr_t *bitmap) 
{
	int i, first, last;
	hostlist_t hl;
	char core_name[20];
	...
	for (i = first; i <= last; i++) {
		...
		sprintf(core_name,"%s%d",L_CORE,i);
		hostlist_push_host(hl, core_name);
	}
	return hl;

}

hostlist_t _bitmap2socket_hostlist (bitstr_t *bitmap) 
{
	int i, first, last;
	hostlist_t hl;
	char socket_name[20];
	...
	for (i = first; i <= last; i++) {
		if (bit_test(bitmap, i) == 0)
			continue;
		sprintf(socket_name,"%s%d",L_SOCKET,i);
		hostlist_push_host(hl, node_record_table_ptr[i].name);
	}
	return hl;
...
char * _hostlist2name_sortable (hostlist_t hl, bool sort)
{
	char *buf;
	if (hl == NULL)
		return xstrdup("");
...
int _add_job_l(struct job_resources *job, uint32_t job_id)
{
	/* core will be updated in the Layouts */	
	int socket_begin,socket_end,core_begin,core_end,node_begin,node_end;
	
	uint32_t allocated_core_count=1,allocated_socket_count=1,allocated_node_count=1,
		allocated_threads_per_core;
	
	int i,j,k,slurm_status=SLURM_SUCCESS;
	char str_i[30],*parent_name,*pre_parent_name="-1";
	char core[145],node[41];	

	int core_node_begin,core_node_end,num_of_cores_node,core_offset;
		
	node_begin = bit_ffs(job->node_bitmap);
			
	if (core_begin >= 0)
		core_end = bit_fls(job->core_bitmap);
	else
		core_end = core_begin - 1;
	
	if (node_begin >= 0)
		node_end = bit_fls(job->node_bitmap);
	else
	...
	j=0;
	for(i=node_begin; i<=node_end ; i++)
	{
		info("Entrance -1");
		if( !bit_test(job->node_bitmap,i))
			continue;
		core_node_begin=cr_get_coremap_offset (i); // node i's core begin
		core_node_end = cr_get_coremap_offset (i+1);
		num_of_cores_node = core_node_end - core_node_begin ;
		info("Entrance -2 num_of_cores_node:%d",num_of_cores_node);
		for(k=0; (j <= core_end) && (k < num_of_cores_node); k++,j++)
		{
			info("L-Job_res core is going to allocate j:%d,k:%d",j,k);
			if(bit_test(job->core_bitmap,j))
			{
				info("L-Job_res core is going to allocate");
				allocated_threads_per_core = (uint32_t)node_record_table_ptr[i].threads;
				core_offset = core_node_begin + k;
				if( core_offset < core_node_end)
				{
					sprintf(str_i,"%s%d",L_CORE,core_offset);
					info("L-Job_res core:%s is going to allocate core_offset:%d allocated_threads_per_core:%d",str_i,core_offset,allocated_threads_per_core);
					if(layouts_entity_set_kv (L_CONS_RES_PARTITION,str_i,
										              L_ALLOCATED_CORE_COUNT,&allocated_core_count,
										              L_T_UINT32) == SLURM_SUCCESS &&
					   layouts_entity_set_kv (L_CONS_RES_PARTITION,str_i,
										              L_ALLOCATED_THREADS_PER_CORE,&allocated_threads_per_core,
										              L_T_UINT32) == SLURM_SUCCESS &&
					   layouts_entity_set_kv (L_CONS_RES_PARTITION,str_i,
										              L_JOB_ID,&job_id,
										              L_T_UINT32) == SLURM_SUCCESS)
					{
						info("L-Job_res core:%s is allocated:%d allocated_threads_per_core:%d",str_i,allocated_core_count,allocated_threads_per_core);
					}
					else 
						return SLURM_ERROR;
					
					parent_name = layouts_entity_get_parent_name (L_CONS_RES_PARTITION,str_i);
					
					info("Socket is going to set:%s",parent_name);
					if(parent_name)
					{
						if( pre_parent_name && strcmp(parent_name,pre_parent_name) != 0 )
						{
							if(layouts_entity_set_kv (L_CONS_RES_PARTITION,parent_name,L_ALLOCATED_SOCKET_COUNT,&allocated_socket_count,L_T_UINT32)==SLURM_SUCCESS)
							{
								info("Socket is allocated");
							}
							pre_parent_name = parent_name;
						}
					}
				}
	...
	info("_add_job_l return:%d",slurm_status);
	return slurm_status;
}

void * _add_job_parallel(void *tp_index)
{
	/* core will be updated in the Layouts */	
	int socket_begin,socket_end,core_begin,core_end,node_begin,node_end;
	...
	char core[145],node[41];
	int core_node_begin,core_node_end,num_of_cores_node,core_offset;
	int index = (int )(tp_index);
	
	struct job_record *job = tps[index].job;	
	int start = tps[index]. start;
	int end = tps[index].end;
	info("thread:%d start:%d,end:%d Job:%d",index,start,end,job->job_id);

	bit_fmt(str_i,sizeof(str_i),job->job_resrcs->node_bitmap);
		info("  Node-map:%s",str_i);
	bit_fmt(str_i,sizeof(str_i),job->job_resrcs->core_bitmap);
		info("  Core-map:%s",str_i);
	bit_fmt(str_i,sizeof(str_i),job->job_resrcs->core_bitmap_original_job);
		info("  core_bitmap_original_job:%s",str_i);
			
	for(i=start; i< end ; i++)
	{
		info("i:%d",i);
		if( ! bit_test(tps[index].job->job_resrcs->node_bitmap,i))
			continue;
		core_node_begin=cr_get_coremap_offset (i); // node i's core begin
		core_node_end = cr_get_coremap_offset (i+1);
		
		for(j=core_node_begin; j < core_node_end ; j++)
		{
			info("j:%d",j);
			if(! bit_test(job->job_resrcs->core_bitmap_original_job,j) )
				continue ;
			sprintf(str_i,"%s%d",L_CORE,j);
			info("L-Job_res core:%s is going to allocate ",str_i);
			if(layouts_entity_parallel_set_kv (L_CONS_RES_PARTITION,str_i,
						L_ALLOCATED_CORE_COUNT,&allocated_core_count,
						L_T_UINT32) == SLURM_SUCCESS &&
				layouts_entity_parallel_set_kv (L_CONS_RES_PARTITION,str_i,
						L_ALLOCATED_THREADS_PER_CORE,&node_record_table_ptr[i].threads,
						L_T_UINT32) == SLURM_SUCCESS &&
				layouts_entity_parallel_set_kv (L_CONS_RES_PARTITION,str_i,
						L_JOB_ID,&job->job_id,L_T_UINT32) == SLURM_SUCCESS)
			{
				info("L-Job_res core:%s is allocated:%d allocated_threads_per_core:%d",str_i,allocated_core_count,allocated_threads_per_core);
			}
			else 
			...
			if(parent_name)
			{
				if( pre_parent_name && strcmp(parent_name,pre_parent_name) != 0 )
				{
					if(layouts_entity_parallel_set_kv (L_CONS_RES_PARTITION,parent_name,L_ALLOCATED_SOCKET_COUNT,&allocated_socket_count,L_T_UINT32)==SLURM_SUCCESS)
					{
						info("Socket is allocated");
					}
					pre_parent_name = parent_name;
	...
	info("_add_job_parallel return:%d",slurm_status);
	pthread_exit((void*) tp_index);
}

void* _add_job_serial_multiset(void *job_rec)
{
	/* core will be updated in the Layouts */	
	...
	struct job_record *job_rec_t = (struct job_record *)job_rec;
	struct job_resources *job =job_rec_t -> job_resrcs;
	...
	info("before bitmap2core_socket_list"); 
		
	int str_size;
	char core_name[20];
	char multi_key[100];
	sprintf(multi_key,"%s,%s",L_ALLOCATED_CORE_COUNT,L_JOB_ID);
	uint32_t multi_value[2];
	size_t length = 2* sizeof(uint32_t);
	int ne = 0;
	
	for( j=node_begin ; j <= node_end ; j++ )
	{
		if( !bit_test(job->node_bitmap,j) )
			continue;
		core_node_begin = cr_get_coremap_offset (j);
		core_node_end = cr_get_coremap_offset (j+1);
	
		for( i=core_node_begin ; i < core_node_end ; i++ )
		{			
			if( !bit_test(job->core_bitmap_original_job,i) )
				continue;
			else
			{
				multi_value[0] = 1;
				multi_value[1] = job_rec_t-> job_id;
				//info ("Enter - %d",i);			
				sprintf(core_name, "%s%d",L_CORE,i);				
				ne = layouts_entity_set_mkv(L_CONS_RES_PARTITION,core_name,multi_key,&multi_value,length,L_T_UINT32);
				//info("L_ALLOCATED_CORE_COUNT:%d",ne);
								
			}					
		}
	}
			
/*	info("before add pull");
	if(layouts_entity_pull_kv (L_CONS_RES_PARTITION,L_CLUSTER,
	   "AllocatedSumNodes")==SLURM_SUCCESS)
	{
...
void* _add_job_serial_parallel(void *job_rec)
{
	/* core will be updated in the Layouts */	
	int socket_begin,socket_end,core_begin,core_end,node_begin,node_end;
	...
	/*#pragma omp parallel shared(i)
	{
		...
		for(i=node_begin; i<=node_end ; i++)
		{
			//info("Entrance -1 tid:%d",omp_get_thread_num());
			if( !bit_test(job->node_bitmap,i))
				continue;
			core_node_begin=cr_get_coremap_offset (i); // node i's core begin
			core_node_end = cr_get_coremap_offset (i+1);
			...
			for(j = core_node_begin; j < core_node_end ; j++)
			{
				info("L-Job_res core is going to allocate j:%d",j);
				if(bit_test(job->core_bitmap_original_job,j))
				{
					info("L-Job_res core is going to allocate");
					allocated_threads_per_core = (uint32_t)node_record_table_ptr[i].threads;
					sprintf(str_i,"%s%d",L_CORE,j);
	...
	info("before add pull");
	if(layouts_entity_pull_kv (L_CONS_RES_PARTITION,L_CLUSTER,L_ALLOCATED_SUM_CORES_CLUSTER)==SLURM_SUCCESS)
	{
		if(layouts_entity_pull_kv (L_CONS_RES_PARTITION,L_CLUSTER,L_ALLOCATED_SUM_NODES_CLUSTER)==SLURM_SUCCESS )
		{
			info("L-Job_res after add pull Cluster level allocated_sum_cores:%d allocated_sum_sockets:%d allocated_sum_nodes:%d",
	...
	info("_add_job_serial_parallel return:%d",slurm_status);
	int tp_index =1 ;
	pthread_exit((void*) tp_index);
}


int _add_job_ll(struct job_resources *job, uint32_t job_id)
{
	/* core will be updated in the Layouts */	
	int socket_begin,socket_end,core_begin,core_end,node_begin,node_end;
			       ...
			       struct job_record *job_ptr)
{
	...
	//info("cons_res: build_row_bitmaps reshuffling %u jobs", num_jobs);
	...
	if (j < num_jobs) {
		...
		//info("cons_res: build_row_bitmap: dangling job found");
 ...
 * whatever will be changed here will be updated in the _rm_job_from_res also, 
 * both are isomorphic functions.
 */
static int _add_job_to_res(struct job_record *job_ptr, int action)
{
	...
	List gres_list;
	// variable for threads and core_bitmap_original_job allocation 
	int i, n,rc,status,start,end,node_begin; 
	bitstr_t *core_bitmap;
	
	pthread_t thread_serial; /* Multi thread is possible by using array of threads, 
	 * but here it is a single threadesd one. * */
	pthread_attr_t attr;
	...
	node_begin = bit_ffs(job->node_bitmap);
	int nodes = bit_size(job->node_bitmap);
	int size = cr_get_coremap_offset(nodes);
	
	job->core_bitmap_original_job = bit_alloc(size);
	bit_shift_copy(job->core_bitmap_original_job,job->core_bitmap,cr_get_coremap_offset (node_begin));
	
	pthread_attr_init(&attr);
	pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE);
	rc = pthread_create(&thread_serial, &attr,_add_job_serial_multiset, (void *)job_ptr); 
		  		if (rc) 
				{
					 error("add-job:return code from pthread_create() is %d\n", rc);
				}
	/* Even multi thread within the above single thread is possible but
	 * functionality assurance will lose for sometimes, so we used single thread 
	 * and have the optimized layouts_entity_multiset operation 
	 * 
	 */	

	debug3("Start of normal job add");

	/*char str_i[40];
	bit_fmt(str_i,sizeof(str_i)*40,job->node_bitmap);
	info("Add job node allocated:%s",str_i);
	bit_fmt(str_i,sizeof(str_i)*40,job->core_bitmap_original_job);
	info("Add job Original Core allocated:%s",str_i);
	bit_fmt(str_i,sizeof(str_i)*40,job->core_bitmap);
	info("Add job Core allocated:%s",str_i);*/
	...
	/* pthead fork and join model used for parallel implicit, Here is the join model.
	 * If you used multi thread those multi threads should be joined here
	 */
	rc = pthread_join(thread_serial, &status);
			if (rc) 
			{
				error("add-job:return code from pthread_join() is %d\n ststus:%d", rc,status);
			}
...
void* _rm_job_serial_multiset(void *job_rec)
{
/* core will be updated in the Layouts */	
	int node_begin,node_end;
...
/*	info("before add pull");
	if(layouts_entity_pull_kv (L_CONS_RES_PARTITION,L_CLUSTER,
	   "AllocatedSumNodes,AllocatedSumCoresInCluster,AllocatedSumSocketsInCluster")==SLURM_SUCCESS)
	{
...
int _rm_job_l(struct job_resources *job)
{
	...
	uint32_t allocated_sum_cores_socket,num_sum_cores_socket,allocated_sum_cores_node,
		num_sum_cores_node,allocated_sum_sockets_node,num_sum_sockets_node,
		switch_to_process_clear=0;
	uint32_t allocated_core_count=0,allocated_socket_count=0,allocated_node_count=0,
		allocated_threads_per_core=0,job_id=0;
	int i,j,k,slurm_status=SLURM_SUCCESS;
	char str_i[30],*parent_name,*pre_parent_name = "-1";
	char core[145],node[41];	
	...
	core_begin = cr_get_coremap_offset (node_first_set);
	socket_begin = cr_get_socketmap_offset (node_first_set);
	...
	for(i=node_first_set; i<=node_last_set ; i++)
	{
		...
		for(k=0; (j <= core_end) && (k < num_of_cores_node); k++,j++)
		{
			info("L-Job_res core is going to deallocate j:%d,k:%d",j,k);
			if(bit_test(job->core_bitmap,j))
			{
				info("L-Job_res core is going to deallocate");
				core_offset = core_node_begin + k;
				if( core_offset < core_node_end)
				{
					sprintf(str_i,"%s%d",L_CORE,core_offset);
					info("L-Job_res core:%s is going to deallocate core_offset:%d",str_i,core_offset);
					if(layouts_entity_set_kv (L_CONS_RES_PARTITION,str_i,
										              L_ALLOCATED_CORE_COUNT,&allocated_core_count,
										              L_T_UINT32) == SLURM_SUCCESS &&
					   layouts_entity_set_kv (L_CONS_RES_PARTITION,str_i,
										              ...
										              L_T_UINT32) == SLURM_SUCCESS)
					{
						...
						return SLURM_ERROR;
					parent_name = layouts_entity_get_parent_name (L_CONS_RES_PARTITION,str_i);
					info("Socket is going to clear:%s",parent_name);
					if(parent_name)
					{
						if( pre_parent_name == NULL || (pre_parent_name && strcmp(parent_name,pre_parent_name) != 0))
						{
							if(layouts_entity_set_kv (L_CONS_RES_PARTITION,parent_name,L_ALLOCATED_SOCKET_COUNT,&allocated_socket_count,L_T_UINT32)==SLURM_SUCCESS)
							{
								info("Socket is cleared");
							}
							pre_parent_name = parent_name;
						}
					}
	...
	{
		info("L-Job_res Cluster level allocated_sum_cores:%d ",allocated_sum_cores);
	}
	else 
		return SLURM_ERROR;
	...
	for(i=node_first_set ; i<=node_last_set ; i++) 
	{
		if( bit_test(job->node_bitmap,i) )
		{
			...
			for(j=socket_begin; j<socket_end; j++)
			{
				...
				if(layouts_entity_get_kv(L_CONS_RES_PARTITION,str_i,L_ALLOCATED_SUM_CORES_SOCKET,
					                         &allocated_sum_cores_socket,L_T_UINT32)==SLURM_SUCCESS)
				{
					if( allocated_sum_cores_socket == 0 &&
					   layouts_entity_set_kv (L_CONS_RES_PARTITION,str_i,L_ALLOCATED_SOCKET_COUNT,
			...
			if(layouts_entity_get_kv(L_CONS_RES_PARTITION,node_record_table_ptr[i].name,L_ALLOCATED_SUM_CORES_NODE,
					                         &allocated_sum_cores_node,L_T_UINT32)==SLURM_SUCCESS &&
				layouts_entity_get_kv(L_CONS_RES_PARTITION,node_record_table_ptr[i].name,L_NUM_SUM_CORES_NODE,
	...
	{
		info("L-Job_res Cluster level  allocated_sum_sockets:%d allocated_sum_nodes:%d",
		     allocated_sum_sockets,allocated_sum_nodes);
	}
	else 
		return SLURM_ERROR;*/
	/*info("start rm_job_l pullget");

	if(layouts_entity_pullget_kv (L_CONS_RES_PARTITION,L_CLUSTER,L_ALLOCATED_SUM_CORES_CLUSTER,&allocated_sum_cores,L_T_UINT32)==SLURM_SUCCESS)
	{
		if(layouts_entity_pullget_kv (L_CONS_RES_PARTITION,L_CLUSTER,L_ALLOCATED_SUM_NODES_CLUSTER,&allocated_sum_nodes,L_T_UINT32)==SLURM_SUCCESS )
			{
	...
	info("_rm_job_l return:%d",slurm_status);
	return slurm_status;
}
			    ...
			    struct job_record *job_ptr, int action)
{
	...
	int status,node_begin = bit_ffs(job->node_bitmap);
	int nodes = bit_size(job->node_bitmap);
	int size = cr_get_coremap_offset(nodes);
	...
	pthread_attr_init(&attr);
	pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE);
	int rc = pthread_create(&thread_serial, &attr,_rm_job_serial_multiset, (void *)job_ptr); 
		  		if (rc) 
				{
					 info("ERROR:return code from pthread_create() is %d\n", rc);
				}
	
	//info("cons_res: _rm_job_from_res: job %u action %d", job_ptr->job_id,action);
		...
		for (i = 0; i < p_ptr->num_rows; i++) {
			...
			for (j = 0; j < p_ptr->row[i].num_jobs; j++) {
				if (p_ptr->row[i].job_list[j] != job)
					continue;
				/*info("cons_res: removed job %u from "
				       "part %s row %u",
				       job_ptr->job_id,
	...
	rc = pthread_join(thread_serial, &status);
			if (rc) 
			{
				info("ERROR; return code from pthread_join() is %d\n ststus:%d", rc,status);
			}
				 ...
				 struct node_record *node_ptr)
{
	...
	//info("cons_res: _rm_job_from_one_node: job %u node %s",job_ptr->job_id, node_ptr->name);
	if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE)
		_dump_job_res(job);
	...
	for (i = first_bit, n = -1; i <= last_bit; i++) {
		...
		if (job->cpus[n] == 0) {
			//info("attempt to remove node %s from job %u again",node_ptr->name, job_ptr->job_id);
			return SLURM_SUCCESS;
		}
	...
	for (i = 0; i < p_ptr->num_rows; i++) {
		...
		for (j = 0; j < p_ptr->row[i].num_jobs; j++) {
			if (p_ptr->row[i].job_list[j] != job)
				continue;
			//info("cons_res: found job %u in part %s row %u",job_ptr->job_id, p_ptr->part_ptr->name, i);
		      ...
		      uint32_t req_nodes, uint16_t job_node_req)
{
	...
	if (job_ptr->part_ptr->cr_type) {
		if ((cr_type & CR_SOCKET) || (cr_type & CR_CORE)) {
			...
			//info("cons_res: Can't use Partition SelectType unless using CR_Socket or CR_Core");
		}
	}
	...
	if (rc != SLURM_SUCCESS) {
		...
		while ((tmp_job_ptr = list_next(job_iterator))) {
			...
			if (ovrlap == 0)	/* job has no usable nodes */
				continue;	/* skip it */
			//info("cons_res: _will_run_test, job %u: overlap=%d",tmp_job_ptr->job_id, ovrlap);
			_rm_job_from_res(future_part, future_usage,
					 tmp_job_ptr, 0);
			if(LAYOUT)
				rc = cr_job_test_l(job_ptr, bitmap, min_nodes,
...
extern int select_p_node_init(struct node_record *node_ptr, int node_cnt)
{
	...
	int socket_begin,socket_end,selected_sum_nodes,selected_sum_cpus;
	char str_i[30];
	uint16_t sockets;
	...
	/*cons_res_topology_buf = init_buf(134217727);
	info("Cons_res topology buffering started ...");
	if(layouts_pack_layout (L_CONS_RES_TOPOLOGY,cons_res_topology_buf) == SLURM_SUCCESS)
	{
		info("Cons_res topology buffered");
	}

	info("Cons_res topology buffering restore started ...");
    if(layouts_update_layout (L_CONS_RES_TOPOLOGY,cons_res_topology_buf) == SLURM_SUCCESS)
    {
		info("Cons_res topology buffer restored");
    }*/
	
	/* switch_record_table initialization */
	root_switch_index = -2;
	leaf_switch_cnt = -2;
	switch_levels_max = switch_levels + 1;
	/*  level_switch_count(1D) gives the each levels number of switches 
	 * level_switch_index(2D) gives the each levels switches corresponding index in switch_record_table 
	 * */
	level_switch_count = xmalloc(sizeof(int)* (switch_levels+1));
	level_switch_index = xmalloc((sizeof(int *)) *(switch_levels + 1));
	int * level_switch_count_tmp = xmalloc(sizeof(int)* (switch_levels+1));
	
	for( i=0 ; i< switch_record_cnt ; i++)
	{
		level_switch_count[switch_record_table[i].level] ++ ;
		level_switch_count_tmp[switch_record_table[i].level] ++ ;
	...
	for(i=0; i < switch_levels_max ; i++ )
	{
		//info("Switch count:%d in Level:%d",level_switch_count[i],i);
		level_switch_index[i] = xmalloc(sizeof(int)*level_switch_count[i]);
	}
		
	/*char str2_i[30];
	sprintf(str2_i,"%s%d",L_SELECTED_SUM_CPUS_IN_LEVEL,switch_record_table[root_switch_index].level);
	sprintf(str_i,"%s%d",L_SELECTED_SUM_NODES_IN_LEVEL,switch_record_table[root_switch_index].level);
	if(layouts_entity_pullget_kv (L_CONS_RES_TOPOLOGY,switch_record_table[root_switch_index].name,str_i,&selected_sum_nodes,L_T_UINT32) == SLURM_SUCCESS &&
	   layouts_entity_pullget_kv (L_CONS_RES_TOPOLOGY,switch_record_table[root_switch_index].name,str2_i,&selected_sum_cpus,L_T_UINT32) == SLURM_SUCCESS
	   )
	{
	}*/
	...
	for( i=0 ; i < switch_record_cnt ; i++ )
	{
		//info("L-level_switch_count_tmp[switch_record_table[i].level]:%d i:%d ",level_switch_count_tmp[switch_record_table[i].level],i);

		if(level_switch_count_tmp[switch_record_table[i].level] >= 0)
		{
			level_index_tmp = level_switch_count[switch_record_table[i].level] - level_switch_count_tmp[switch_record_table[i].level];
			level_switch_index[switch_record_table[i].level][level_index_tmp] = i;
			info("switch_record_table[i].level:%d level_switch_index[switch_record_table[i].level][level_index_tmp]:%d level_index_tmp:%d",
		    switch_record_table[i].level,level_switch_index[switch_record_table[i].level][level_index_tmp],level_index_tmp);
			level_switch_count_tmp[switch_record_table[i].level] -- ;
		}
		else
		{
			//info("Switch record count and level information is not snchronised");
		}
	}

	for( i=0; i<switch_levels_max ; i++ )
	{
		for( j=0; j < level_switch_count[i] ; j++ )
		{
			info("Switch level:%d index:%d",i,level_switch_index[i][j]);
		}
	}
	...
	if(layouts_entity_pull_mkv (L_CONS_RES_PARTITION,L_CLUSTER,"NumSumNodes,NumSumCoresInCluster,NumSumSocketsInCluster") == SLURM_SUCCESS &&
		   layouts_entity_get_kv (L_CONS_RES_PARTITION,L_CLUSTER,L_NUM_SUM_CORES_CLUSTER,&num_sum_cores,L_T_UINT32) == SLURM_SUCCESS &&
		   layouts_entity_get_kv (L_CONS_RES_PARTITION,L_CLUSTER,L_NUM_SUM_SOCKETS_CLUSTER,&num_sum_sockets,L_T_UINT32) == SLURM_SUCCESS
		   )
	
	{
		info("pullget is succeded");
		
		if(num_sum_nodes == node_record_count)
		{
			error("Layout is upto date with the Node records details- Illegal activities of Modification,Insert&Delete are not acceptable right now");
		}
		else
		{
...
// Core,Socket Bitmap position Correlation with Layout
		...
		for(j=0 ; j< socket_end ; j++)
		{
			...
			{
				//info("Layouts Socket Bitmap Position for Node-%d:%s and Socket:%s was updated correctly",i,node_ptr[i].name,str_i);
			}
		}*/
	...
	/*uint32_t switch_to_process=1;
	sprintf(str_i,"%s%d",L_SWITCHES_TO_PROCESS_IN_LEVEL,switch_record_table[root_switch_index].level);
	if(layouts_entity_set_kv (L_CONS_RES_TOPOLOGY,switch_record_table[switch_record_cnt-3].name,str_i,&switch_to_process,L_T_UINT32) == SLURM_SUCCESS)
	{
		//info("L-switch_to_process:%d.. switch_record_table[switch_record_cnt-3].name:%s",
		     switch_to_process,switch_record_table[switch_record_cnt-3].name);
	}
	
	if(layouts_entity_setpush_kv (L_CONS_RES_TOPOLOGY,switch_record_table[root_switch_index].name,str_i,&switch_to_process,L_T_UINT32) == SLURM_SUCCESS)
	{
		//info("L-switch_to_process:%d.. switch_record_table[root_switch_index].name:%s",switch_to_process,switch_record_table[root_switch_index].name);
	}

	switch_to_process=1;
	sprintf(str_i,"%s%d",L_SWITCHES_TO_PROCESS_IN_LEVEL,switch_record_table[switch_record_cnt-3].level);
	...
	uint32_t switch_to_process_clear = 0;
	uint16_t cpu_clear =0 ;
	sprintf(str_i,"%s%d",L_SWITCHES_TO_PROCESS_IN_LEVEL,switch_record_table[root_switch_index].level);
	if(layouts_entity_setpush_kv(L_CONS_RES_TOPOLOGY,switch_record_table[root_switch_index].name,str_i,&switch_to_process_clear,L_T_UINT32) == SLURM_SUCCESS)
	{
		//info("L- root_switch_index:%d switch_to_proces:%d",root_switch_index,switch_to_process_clear);
	}
	...
	if(layouts_entity_setpush_kv(L_CONS_RES_TOPOLOGY,switch_record_table[root_switch_index].name,str_i,&switch_to_process_clear,L_T_UINT32) == SLURM_SUCCESS)
	{
		//info("L- root_switch_index:%d Selected Nodes cleared:%d",root_switch_index,switch_to_process_clear);
	}

	
	sprintf(str_i,"%s%d",L_SELECTED_SUM_CPUS_IN_LEVEL,switch_record_table[root_switch_index+1].level);
	if(layouts_entity_setpush_kv(L_CONS_RES_TOPOLOGY,switch_record_table[root_switch_index+1].name,str_i,&cpu_clear,L_T_UINT16) == SLURM_SUCCESS)
	{
		//info("L- root_switch_index:%d Selected Cpus cleared:%d",root_switch_index,switch_to_process_clear);
	}

	for(i=0; i < switch_record_table[root_switch_index+1].num_switches ; i++)
	{
		j=switch_record_table[root_switch_index+1].switch_index[i];
		//info("L-Switch root_switch_index+1:%s root_switch_index+1:%d j:%d",switch_record_table[root_switch_index+1].name,root_switch_index+1,j);
	}*/
	_create_part_data();

	//info("L-Check type assignment");
		uint16_t a=1,b=2;
		uint32_t c=3,d=4;
		uint64_t e=5,f=6;
		//info("L-Type Size %d,%d,%d",sizeof(uint16_t),sizeof(uint32_t),sizeof(uint64_t));
		a=c;
		b=e;
		//info("L-16 a:%d b:%d",a,b);
		c=b; d=f;
		//info("L-32 c:%d d:%d",c,d);
		e=a; f=c;
		//info("L-64 e:%d f:%d",e,f);
	info("get parent name");
	char *name;
	name = layouts_entity_get_parent_name (L_CONS_RES_PARTITION,"virtual0");
	info("Parent Name: %s",name);

	hostlist_t core_list = hostlist_create (NULL);
	hostlist_push_host(core_list,"core0") ;
	hostlist_push_host(core_list,"core8") ;
	hostlist_push_host(core_list,"core1000") ;
	hostlist_push_host(core_list,"core100") ;
	hostlist_push_host(core_list,"core3800") ;
	List value = list_create(slurm_destroy_uint32_ptr) ;
	uint32_t val = 12;
	uint32_t valu[4];
	for( i=0 ; i<4 ; i++ )
	{
		list_append(value,val);
		valu[i] = val++;
	}

	/*info("Before multi value set");
	layouts_multi_entity_set_k_multi_v_uint32 (L_CONS_RES_PARTITION,core_list,L_ALLOCATED_THREADS_PER_CORE,valu);
	info("After multi value set");*/

	/*layouts_multi_entity_set_sk_mv(L_CONS_RES_PARTITION, "virtual[0-3]", L_ALLOCATED_NODE_COUNT, 
                valu,sizeof(uint32_t)*4,L_T_UINT32);
	info("After multi value set-2");
	uint32_t value1[4];
	layouts_multi_entity_get_sk_mv(L_CONS_RES_PARTITION, "virtual[0-3]", L_ALLOCATED_NODE_COUNT, 
                value1,sizeof(uint32_t)*4,L_T_UINT32);
	for(i=0 ; i < 4 ; i++ )
	{
		info("multi get value[i:%d]:%d",i , value1[i]);
	}
	info("Before multi set");
	uint32_t set=1;
	
	layouts_multi_entity_set_kv (L_CONS_RES_PARTITION,core_list,L_ALLOCATED_CORE_COUNT,&set,L_T_UINT32);
	info("After multi set");
	layouts_entity_get_kv (L_CONS_RES_PARTITION,"core0",L_ALLOCATED_CORE_COUNT,&set,L_T_UINT32);
	info("core0 allocated:%d",set);
	layouts_entity_get_kv (L_CONS_RES_PARTITION,"core1",L_ALLOCATED_CORE_COUNT,&set,L_T_UINT32);
	info("core1 allocated:%d",set);
	layouts_entity_get_kv_uint32 (L_CONS_RES_PARTITION,"core0",L_ALLOCATED_CORE_COUNT,&set);
	info("core0 allocated:%d",set);
	set = 0;
	layouts_entity_set_kv_uint32 (L_CONS_RES_PARTITION,"core0",L_ALLOCATED_CORE_COUNT,&set);
	info("After 0 was set core0 allocated:%d",set);*/
	return SLURM_SUCCESS;
}
			     ...
			     bitstr_t *exc_core_bitmap)
{
	...
	info("select_p_job_test for job %u", job_ptr->job_id);
	if (!debug_check) {
		debug_check = true;
	...
	/*info("L-job_details num_tasks:%d min_cpus:%d pn_min_cpus:%d whole_node:%d",job_ptr->details->num_tasks,job_ptr->details->min_cpus,
	     job_ptr->details->pn_min_cpus,job_ptr->details->whole_node);*/
	/* Layout Initial value to check availability */
	/*if(node_record_table_ptr && 
	   //layouts_entity_get_kv (L_CONS_RES_PARTITION,L_CLUSTER,L_ALLOCATED_SUM_CORES_CLUSTER,&allocated_sum_cores,L_T_UINT32) == SLURM_SUCCESS && 
	   layouts_entity_get_kv (L_CONS_RES_PARTITION,L_CLUSTER,L_ALLOCATED_SUM_NODES_CLUSTER,&allocated_sum_nodes,L_T_UINT32) == SLURM_SUCCESS &&
	   layouts_entity_get_kv (L_CONS_RES_PARTITION,L_CLUSTER,L_ALLOCATED_SUM_THREADS_CLUSTER,&allocated_sum_threads,L_T_UINT32) == SLURM_SUCCESS &&
	   //layouts_entity_pullget_kv (L_CONS_RES_PARTITION,L_CLUSTER,L_NUM_SUM_CORES_CLUSTER,&num_sum_cores,L_T_UINT32) == SLURM_SUCCESS && 
	   layouts_entity_get_kv (L_CONS_RES_PARTITION,L_CLUSTER,L_NUM_SUM_NODES_CLUSTER,&num_sum_nodes,L_T_UINT32) == SLURM_SUCCESS &&
	   layouts_entity_get_kv (L_CONS_RES_PARTITION,L_CLUSTER,L_NUM_SUM_THREADS_CLUSTER,&num_sum_threads,L_T_UINT32) == SLURM_SUCCESS
	   )
	{
		free_sum_nodes = num_sum_nodes-allocated_sum_nodes;
		//free_sum_cores = num_sum_cores-allocated_sum_cores;
		free_sum_threads = num_sum_threads - allocated_sum_threads;
		int number_of_tasks ;
		if( job_ptr->details && job_ptr ->details->cpus_per_task)
		{
			number_of_tasks = job_ptr->details->num_tasks;
			number_of_tasks *= job_ptr->details->cpus_per_task;
			//info("L- num_tasks basedon cpus:%d MIN(number_of_tasks,job_ptr->details->min_cpus:%d",number_of_tasks,MIN(number_of_tasks,job_ptr->details->min_cpus) );
		}
		if( free_sum_nodes >= MAX(min_nodes, req_nodes) ) // max_nodes, req_nodes check is right now needed
		{
			if( free_sum_threads >= MAX(number_of_tasks,job_ptr->details->min_cpus) )
				//Heterogeneous cluster we have to change
			{
				job_allowed = true;
				info("L-Job-%d is allowed to Run,min_Nodes:%d,req_nodes:%d min_cpus:%d pn_min_cpus:%d required is in available Nodes:%d Cores:%d Threads:%d",job_ptr->job_id,
			    job_ptr->details->min_nodes,job_ptr->details->req_nodes,job_ptr->details->min_cpus,job_ptr->details->pn_min_cpus,free_sum_nodes,free_sum_cores,free_sum_threads);
			}
			else
			{
				job_allowed = false;
				info("L-Job-%d is not allowed to Run,min_Nodes:%d,req_nodes:%d min_cpus:%d pn_min_cpus:%d required is higher than available Nodes:%d Cores:%d Threads:%d",job_ptr->job_id,
			    job_ptr->details->min_nodes,job_ptr->details->req_nodes,job_ptr->details->min_cpus,job_ptr->details->pn_min_cpus,free_sum_nodes,free_sum_cores,free_sum_threads);
			}
		}
		else
	...
	if ((job_ptr->details->core_spec != (uint16_t) NO_VAL) &&
		...
		//info("Setting Exclusive mode for job %u with CoreSpec=%u",job_ptr->job_id, job_ptr->details->core_spec);
	...
	if (select_debug_flags & DEBUG_FLAG_CPU_BIND) {
		if (job_ptr->job_resrcs)
		{
			...
			//info("No job_resources info for Job-%u SLURM_SUCCESS(rc)=%d",job_ptr->job_id, rc);
		}
	} else if (debug_cpu_bind && job_ptr->job_resrcs) {
		log_job_resources(job_ptr->job_id, job_ptr->job_resrcs);
	}

	info("select_p_job_test return:%d",rc);
			   ...
			   uint32_t *core_cnt, bitstr_t **core_bitmap)
{
	...
	if ((node_cnt) && (core_cnt)) {
		...
		//info("Reserving %u cores across %d nodes",total_core_cnt, node_cnt);
	...
	if (core_cnt) { /* Reservation is using partial nodes */
		...
		while (total_core_cnt) {
			...
			if (local_cores < cores_per_node) {
				//info("Skip node %d (local: %d, needed: %d)",inx, local_cores, cores_per_node);
				continue;
			}
			...
			if (cores_in_node < cores_per_node) {
				//info("Skip node %d (avail: %d, needed: %d)",inx, cores_in_node, cores_per_node);
				continue;
			}

			//info("Using node %d (avail: %d, needed: %d)",inx, cores_in_node, cores_per_node);
			...
			if (cores_in_node) {
				...
				//info("Reservation using %d cores in node %d",cores_in_node, inx);
				     ...
				     bitstr_t **core_bitmap)
{
	...
	for (i = 0; i < switch_record_cnt; i++) {
		...
		//info("switch %d available cores in %d nodes: %d",i, switches_node_cnt[i], switches_cpu_cnt[i]);
	}
	...
	while (rem_nodes > 0) {
		...
		for (i=first; ((i<=last) && (first>=0)); i++) {
			...
			if (*core_bitmap) {
				...
				//info("Testing node %d, core offset %d",i, coff);
				for (j=0; j<cr_node_num_cores[i]; j++){
				...
				//info("Using node %d with %d cores available",i, avail_cores_in_node);
			}
...
fini:	for (i=0; i<switch_record_cnt; i++) {
	...
	if (avail_nodes_bitmap && core_cnt) {
		...
		while (core_cnt[0]) {
			...
			//info("Using node inx %d cores_per_node %d core_cnt %d", inx, cores_per_node, core_cnt[0]);
			coff = cr_get_coremap_offset(inx);
			...
			//info("Node %d has %d available cores", inx,avail_cores_in_node);
