...
strong_alias(bit_shift_copy,	slurm_bit_shift_copy);
...
void 
bit_shift_copy(bitstr_t *core_bitmap_dest,bitstr_t *core_bitmap_src,int start)
{
	_assert_bitstr_valid(core_bitmap_src);
	_assert_bitstr_valid(core_bitmap_dest);
	_assert_bit_valid(core_bitmap_dest,start);
	int src_size = bit_size(core_bitmap_src);
	int dest_size = bit_size(core_bitmap_dest);
	int i,j;
	
	if((src_size + start) <= dest_size)
	{
		for(i=start,j=0; i<dest_size && j<src_size ; i++,j++ )
		{
			if(bit_test(core_bitmap_src,j))
			{
				bit_set(core_bitmap_dest,i);
			}
		}
	}
}
