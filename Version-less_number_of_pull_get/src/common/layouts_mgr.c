...
const size_t size_uint32_t = sizeof(uint32_t);
void free(void*);
...
typedef struct layouts_mgr_st {
	...
	xhash_t *au_keydefs;      /* For autoupdate in the specific pull get KVs */
} layouts_mgr_t;
...
#define _entity_update_kv_helper(type_t, operator)			\
	...
	case S_P_OPERATOR_SET_IF_MIN:					\
		if (*rvalue < *lvalue)					\
		{	   \
			info("MIN:*rvalue:%d < *lvalue:%d",*rvalue, *lvalue); \
			*lvalue = *rvalue;				\
		}	   \
		break;							\
	case S_P_OPERATOR_SET_IF_MAX:					\
		if (*rvalue > *lvalue)					\
		{	   \
			info("MAX:*rvalue:%d > *lvalue:%d",*rvalue, *lvalue); \
			*lvalue = *rvalue;				\
		}	   \
		break;							\
	default:							\
...
int _layouts_entity_pull_mkv(layout_t* l, entity_t* e, char* keys)
{
/*	char *key = NULL;
	hostlist_t kl;
	hostlist_iterator_t kl_i,i_kl;
	int key_count=0;
	layouts_keydef_t *keydef;
	layouts_keyspec_t *keyspec;
	int rc_count =0 ;
	char *layout_name;
	char *key_fullname;
	...
	for (i = 0; i < mgr->plugins_count; i++) {
		if (mgr->plugins[i].layout = l) {
			info("Layout selected name: %s",l->name);
			strcpy(layout_name,l->name);
			rc = SLURM_SUCCESS;
	...
	i_kl = kl_i = hostlist_iterator_create(kl);
	if( rc == SLURM_ERROR )
	{
		rc_count = hostlist_count(kl);
		return rc_count;
	}
	while ((key = hostlist_next(kl_i))) 
	{
		info("1-key:%s",key);
		key_count++;
		strcpy(key_fullname,layout_name);
		strcat(key_fullname,".");
		strcat(key_fullname,key);
		info("1-key_fullname:%s",key_fullname);
		keydef = xhash_get(mgr->keydefs,key_fullname);
		while(keydef->ref_key)
		{
			info("2-keydef->ref_key:%s",keydef->ref_key);
			key_count++;
		}
	}
	info("before xhash_init key_count:%d",key_count);
		
		
		//keyspec = xmalloc(key_count* sizeof(layouts_keyspec_t));
		if(mgr->au_keydefs)
		{
			info("Entered into the au_key_defs section");
			while ((key = hostlist_next(i_kl))) 
			{
				info("3-key:%s",key);
		strcpy(key_fullname,layout_name);
		strcat(key_fullname,".");
		strcat(key_fullname,key);
				info("3-key_fullname:%s",key_fullname);
				keydef = (layouts_keydef_t *)xhash_get(mgr->keydefs,key_fullname);
				if(keydef)
				{
					info("3-keydef is not NULL");
					xhash_add (mgr->au_keydefs,keydef);
					info("3-after xhash_add");
					while(keydef->ref_key)
					{
						info("3-keydef->ref_key:%s",keydef->ref_key);
						keydef = (layouts_keydef_t *)
							xhash_get(mgr->keydefs,keydef->ref_key);
						xassert(keydef);
						xhash_add (mgr->au_keydefs,keydef);
					}
				}
	...
	int count_v =0 ;
	count_v = xhash_count (mgr->au_keydefs);
	info("mgr->au_keydefs: count:%d",count_v);
		if(_layouts_autoupdate_layout_kv(l) == SLURM_ERROR)
		{
			rc_count = hostlist_count(kl);
		}
	hostlist_iterator_destroy(kl_i);
	hostlist_destroy(kl);
	info("Before returning rc_count:%d",rc_count);
	
	// Above implementation is a more advanced implementation should only pull what is necessary
	// instead of forcing a full autoupdate 	
	return rc_count;*/
}
...
int _layouts_entity_set_kv_uint32(layout_t* l, entity_t* e, char* key, void* value)
{
	
	layouts_keydef_types_t real_type;
	char key_keydef[PATHLEN];

	if (l== NULL || e == NULL || value == NULL)
		return SLURM_ERROR;

	real_type = _layouts_entity_get_kv_type(l, e, key);
	if ( real_type != L_T_UINT32)
		return SLURM_ERROR;
	
	_normalize_keydef_key(key_keydef, PATHLEN, key, l->type);
	return entity_set_data(e, key_keydef, value,size_uint32_t);
}

int _layouts_entity_set_kv_uint16(layout_t* l, entity_t* e, char* key, void* value)
{
	...
	real_type = _layouts_entity_get_kv_type(l, e, key);
	if ( real_type != L_T_UINT16)
		return SLURM_ERROR;
	
	_normalize_keydef_key(key_keydef, PATHLEN, key, l->type);
	return entity_set_data(e, key_keydef, value, sizeof(uint16_t));
}
...
int _layouts_entity_get_kv_uint32(layout_t* l, entity_t* e, char* key, void* value)
{	
	layouts_keydef_types_t real_type;
	char key_keydef[PATHLEN];
	char ** pstr;
	...
	return entity_get_data (e,key,value,sizeof(uint32_t));
}

int _layouts_entity_get_kv_uint16(layout_t* l, entity_t* e, char* key, void* value)
{	
	layouts_keydef_types_t real_type;
	char key_keydef[PATHLEN];
	char ** pstr;
	...
	return entity_get_data (e,key,value,sizeof(uint16_t));
}
...
int _layouts_entity_set_mkv(layout_t* l, entity_t* e, char* keys, void* value,
			    size_t length, layouts_keydef_types_t key_type)
{
	char *key = NULL;
	hostlist_t kl;
	...
	while ((key = hostlist_shift(kl))) {
		...
		if (_layouts_entity_get_kv_size(l, e, key, &elt_size) ||
		    (processed + elt_size) > length ||
		    _layouts_entity_set_kv(l, e, key, value, key_type)) {
			rc++;
			processed = length;
			continue;
		}
...
int _layouts_multi_entity_set_kv(layout_t* l, entity_t* e, char* key,
				void* value, size_t length)
{
	layouts_keydef_types_t real_type;
	char key_keydef[PATHLEN];
	...
	return entity_set_data(e,key_keydef,value,length);
		
}

inline int _layouts_multi_entity_get_kv(layout_t* l, entity_t* e, char* key,
				void* value, size_t length)
{
	layouts_keydef_types_t real_type;
	char key_keydef[PATHLEN];
	...
	return entity_get_data(e,key_keydef,value,length);
...
static void layouts_mgr_init(layouts_mgr_t* mgr)
{
	...
	/*mgr->au_keydefs =  xhash_init(layouts_keydef_idfunc,
				  _layouts_keydef_free, NULL, 0);*/ 
}
				 ...
				 void* oldvalue, void* value)
{
	...
	uint32_t *oldval =  oldvalue;
	uint32_t *val = value;	
	//info("oldval:%d , val:%d",*oldval,*val);
 ...
 * helper function used to update KVs of an entity using its xtree_node
 * looking for known inheritance in the neighborhood (parents/children) */
static void _tree_update_node_entity_data_kv(void* item, void* arg) {
	...
	if( mgr->au_keydefs )
	{
		keydef = xhash_get(mgr->au_keydefs, data->key);
	}
	
	//info("Entity-Key:%s ",data->key);
	if(keydef == NULL)
	{
		return;
	}
	//info("keydef-key:%s flags:%d",data->key,keydef->key,keydef->flags);
	/* only work on keys that depend of their neighborhood */
	if (!keydef->flags & KEYSPEC_UPDATE_CHILDREN_MASK &&
	    !keydef->flags & KEYSPEC_UPDATE_PARENTS_MASK) {
		return;
...
static uint8_t _autoupdate_layout_tree_kv(xtree_node_t* node, uint8_t which,
				       uint32_t level, void* arg)
{
	xtree_node_t* child;
	entity_node_t* cnode;
	...
	/*info ("Tree:level:%d which:%d c_node-name:%s type:%s",
	      level,which,cnode->entity->name,cnode->entity->type);*/

	/* iterate over the K/V of the entity, syncing them according
	 * to their autoupdate flags */
	xhash_walk(cnode->entity->data, _tree_update_node_entity_data_kv,
		   &sync_args);
...
int _layouts_autoupdate_layout_kv(layout_t* layout)
{
	/* autoupdate according to the layout struct type */
	switch(layout->struct_type) {
	case LAYOUT_STRUCT_TREE:
		xtree_walk(layout->tree, NULL, 0,
			   XTREE_LEVEL_MAX,
			   _autoupdate_layout_tree_kv, NULL);
		break;
	}
...
{
	layout_t* l =  xhash_get(mgr->layouts, type);
	//info("Layout type:%s and name:%s",type,l->name);
	return l ;
}
...
#define _layouts_entity_wrapper_parallel(func, l, e, r...)			\
	layout_t* layout;						\
	entity_t* entity;						\
	int rc;								\
	layout = layouts_get_layout_nolock(l);				\
...
#define _layouts_multi_entity_wrapper(func, l, e, r...)			\
	layout_t* layout;						\
	entity_t* entity;						\
	slurm_mutex_lock(&mgr->lock);					\
	layout = layouts_get_layout_nolock(l);				\
...
#define _layouts_multi_entity_non_return(func, l, e, r...)			\
	layout_t* layout;						\
	entity_t* entity;						\
	int rc;								\
	slurm_mutex_lock(&mgr->lock);					\
...
int layouts_entity_get_kv_uint32(char* l, char* e, char* key, void* value)
{
	_layouts_entity_wrapper(_layouts_entity_get_kv_uint32, l, e,
				key,value);
}

int layouts_entity_get_kv_uint16(char* l, char* e, char* key, void* value)
{
	_layouts_entity_wrapper(_layouts_entity_get_kv_uint16, l, e,
				key,value);
}

int layouts_entity_set_kv_uint32(char* l, char* e, char* key, void* value)
{
	_layouts_entity_wrapper(_layouts_entity_set_kv_uint32, l, e,
				key,value);
}

int layouts_entity_set_kv_uint16(char* l, char* e, char* key, void* value)
{
	_layouts_entity_wrapper(_layouts_entity_set_kv_uint16, l, e,
				key,value);
}
...
int layouts_entity_set_mkv(char* l, char* e, char* keys, void* value,
			   size_t size, layouts_keydef_types_t key_type)
{
	_layouts_entity_wrapper(_layouts_entity_set_mkv, l, e,
				keys, value, size, key_type);
}
...
int layouts_entity_parallel_get_kv(char* l, char* e, char* key, void* value,
			      layouts_keydef_types_t key_type)
{
	_layouts_entity_wrapper_parallel(_layouts_entity_get_kv, l, e,
				key, value, key_type);
}

int layouts_entity_parallel_set_kv(char* l, char* e, char* key, void* value,
			      layouts_keydef_types_t key_type)
{
	_layouts_entity_wrapper_parallel(_layouts_entity_set_kv, l, e,
				key, value, key_type);
}

char* layouts_entity_get_parent_name(char* l, char* e)
{
	    layout_t *layout = layouts_get_layout (l);
        entity_t *entity = layouts_get_entity (e);
// There is no entity or layout in the LAYOUTS manager chek
        if(layout == NULL || entity == NULL || layout->tree == NULL )
        {
                return NULL;
        }
       // xtree_t *tree_1 = layout_1->tree;
        entity_node_t *c_e_node = entity_get_node (entity,layout);
        xtree_node_t *p_x_node =(xtree_node_t *) (
							((xtree_node_t *)c_e_node->node)->parent);
//If the node has parent or not check
        if( p_x_node == NULL )
        {
                return NULL;
        }
        entity_node_t *enode = (entity_node_t*) p_x_node->data;
        return enode->entity->name;
}

/*int layouts_multi_entity_set_kv(char* layout, hostlist_t entity,
			  char* key, void *value,int count,layouts_keydef_types_t key_type)
{	
	char *entity_name;
	ListIterator li = list_iterator_create(entity);
	int rc ;

	if( entity == NULL )
	{
		info("return slurm error");
		return SLURM_ERROR;
	}
	
	while ((entity_name = list_next(li) ) != NULL)
	{				
		//info("Layout_mgr entity:%s",entity_name);
		 _layouts_multi_entity_wrapper(_layouts_multi_entity_set_kv, l, entity_name,
				key, value, key_type);
		if(rc == SLURM_ERROR)
		{
			info("rc is 0");
...
/*int layouts_multi_entity_set_kv_uint32(char* l,List entity,
...
{	
	...
	{				
		//info("Layout_mgr entity:%s",entity_name);
		 _layouts_multi_entity_wrapper(_layouts_entity_set_kv, l, entity_name,
				key, value, L_T_UINT32);
		if(rc == SLURM_ERROR)
		{
			info("rc is 0");
			return rc;
...
int layouts_multi_entity_set_k_multi_v_uint32 (char* l,List entity,
			  char* key,void *value)
{	
	char *entity_name;
	ListIterator en = list_iterator_create(entity);
/*	int rc = SLURM_ERROR ,i=0;
	uint32_t value_temp ;*/
	int num_entities=0,proc_entities=0;
	size_t elt_size = sizeof(void *);
	bool key_size = true;
	...
	while ((entity_name = list_next(en) ) != NULL )
	{	
		value += elt_size;
		num_entities++;
		if(layouts_entity_set_kv(l, entity_name, key, value, L_T_UINT32) == SLURM_SUCCESS)
		{
			proc_entities ++;
		}
		value += elt_size;
	}
	return (num_entities - proc_entities);
}

int layouts_multi_entity_get_kv(char* layout, hostlist_t entity,
			  char* key, void *value/*,size_t length*/,layouts_keydef_types_t key_type)
{	
	/*char *key = NULL;
	size_t processed = 0;
	size_t elt_size = sizeof(void*);;
	...
	while ((e = hostlist_shift(entity))) 
	{
		if( first == true )
		{
			if (layouts_entity_get_kv_size(l, entity, key, &elt_size ) == SLURM_SUCCESS )
			{
				first = false ;
			}
			else
		...
		if ((processed + elt_size) > length ||
		    layouts_entity_get_kv(layout, e, key, value, key_type)) 
		{
			rc++;
			processed = length;
			continue;
	...
	hostlist_destroy(entity);
	return rc;*/	
}

int layouts_multi_entity_set_kv(char* layout, hostlist_t entity,
			  char* key, void *value,layouts_keydef_types_t key_type)
{	
	int rc = 0;
	char *e ;
	bool first = true;
	...
	while ((e = hostlist_shift(entity))) 
	{
		//info("multi entity host:%s",e);
		if (layouts_entity_set_kv(layout, e, key, value, key_type) ==
											SLURM_ERROR) 
		{
			rc++;
		}
...
int layouts_multi_entity_set_kv_uint32(char* layout, hostlist_t entity,
			  char* key, void *value)
{	
	int rc = 0;
	char *e ;
	...
	{
		//info("multi entity host:%s",e);
		if (layouts_entity_set_kv_uint32(layout, e, key, value) ==
											SLURM_ERROR) 
		{
			rc++;
		}
...
{	
	...
	{			
		...
		if ((processed + elt_size) > length ||
		    (layouts_entity_set_kv(l,entity, key, value, key_type) == SLURM_ERROR)) 
		{
			rc++;
			processed = length;
			continue;
...
int layouts_multi_entity_get_sk_mv(char* l, char *entities, char* key, 
                void* value,size_t length,layouts_keydef_types_t key_type)
{	
	char *entity = NULL;
	hostlist_t el;
	...
	{			
		...
		if ((processed + elt_size) > length ||
		    (layouts_entity_get_kv(l,entity, key, value, key_type) == SLURM_ERROR)) 
		{
			rc++;
			processed = length;
			continue;
...
int layouts_entity_pull_mkv(char* la, char* e, char* keys)
{
	//_layouts_entity_wrapper(_layouts_entity_pull_mkv, la, e, keys);
	char *key = NULL;
	hostlist_t kl;
	hostlist_iterator_t kl_i,i_kl;
	int key_count=0;
	...
	int rc_count =0 ;
	char *layout_name;
	char key_fullname[PATHLEN];
	xhash_t *auto_update_keydefs;
	
	layout_t *l = layouts_get_layout_nolock(la);

	int i, rc = SLURM_ERROR;
	/* look if the corresponding layout plugin enables autoupdate */
	for (i = 0; i < mgr->plugins_count; i++) {
		if (mgr->plugins[i].layout = l) {
			//info("Layout selected name: %s",l->type);
			layout_name = l->type;
			rc = SLURM_SUCCESS;
			/* no autoupdate allowed, return success */
			if (!mgr->plugins[i].ops->spec->autoupdate)
...
/*	while ((key = hostlist_next(kl_i))) 
	{
		info("1-key:%s",key);
		key_count++;
		_normalize_keydef_key(key_fullname, PATHLEN, key,
				      l->type);
		info("1-key_fullname:%s",key_fullname);
		keydef = xhash_get(mgr->keydefs,key_fullname);
		info("1-key-once-again:%s",keydef->key);
		xassert(keydef);
		while(keydef->ref_key != NULL)
		{
			info("2-keydef->ref_key:%s",keydef->ref_key);
			key_count++;
			keydef = (layouts_keydef_t *)
	...
	info("before xhash_init key_count:%d",key_count);*/
		...
		//keyspec = xmalloc(key_count* sizeof(layouts_keyspec_t));
	/*if(mgr->au_keydefs == NULL )
	{
		info("Beofre initialize has Table");
		mgr->au_keydefs =  xhash_init(layouts_keydef_idfunc,
				  _layouts_keydef_free, NULL, 0);
	}*/
	auto_update_keydefs =  xhash_init(layouts_keydef_idfunc,
				  _layouts_keydef_free, NULL, 0);
	
		if(auto_update_keydefs)
		{
			//info("Entered into the au_key_defs section");
			while ((key = hostlist_next(i_kl))) 
			{
		...
		_normalize_keydef_key(key_fullname, PATHLEN, key,
				...
				{
					//info("3-keydef is not NULL");
					xhash_add (auto_update_keydefs,keydef);
					//info("3-after xhash_add");
					while(keydef->ref_key != NULL)
					{
						//info("3-keydef->ref_key:%s",keydef->ref_key);
	...
	int count_v =0 ;
	mgr->au_keydefs = auto_update_keydefs;
	count_v = xhash_count (mgr->au_keydefs);
	info("mgr->au_keydefs: count:%d",count_v);
		if(_layouts_autoupdate_layout_kv(l) == SLURM_ERROR)
		{
	...
	info("Beofre free has Table");
	xfree(auto_update_keydefs);
	hostlist_iterator_destroy(kl_i);
	hostlist_destroy(kl);
	//info("Before returning rc_count:%d",rc_count);
