...
#define L_SELECTED_REQUIRED_IN_LEVEL "SwitchesRequiredInLevel" //Network
#define L_SELECTED_REQUIRED_IN_NODE "SwitchesRequiredCount" //Network
...
static inline uint16_t _allocate_sc(struct job_record *job_ptr, bitstr_t *core_map,
			      bitstr_t *part_core_map, const uint32_t node_i,
			      bool entire_sockets_only,bool config) __attribute__((always_inline));
inline uint16_t _can_job_run_on_node(struct job_record *job_ptr, bitstr_t *core_map,
			      const uint32_t node_i,
			      struct node_use_record *node_usage,
			      uint16_t cr_type,
			      bool test_only, bitstr_t *part_core_map,bool config)  __attribute__((always_inline));
 ...
 */
inline uint16_t _allocate_sockets(struct job_record *job_ptr, bitstr_t *core_map,
 ...
 */
inline uint16_t _allocate_cores(struct job_record *job_ptr, bitstr_t *core_map,
			 bitstr_t *part_core_map, const uint32_t node_i,
			      ...
			      bool entire_sockets_only,bool config)
{
	...
	/* // Layouts based implementation, But it will not support preemption, 
	 // Because LAYOUT has to copied before remove cores allocated from LAYOUTS
	int socket_begin = cr_get_socketmap_offset (node_i);
	int socket_end = cr_get_socketmap_offset (node_i+1);
	uint32_t allocated_sum_cores_socket,free_sum_cores_socket,
			allocated_socket_count;
	char str_i[20];
	if( config == true )
	{
		for (i = 0 ; i < sockets ; i++) 
		{			
			...
			free_cpu_count += cores_per_socket*threads_per_core;			
		}
		used_cpu_count =0;		
	}
	else
...
/* clear nodes already allocated or not satisfied the job's request */
static int clear_nodes_allocated(bitstr_t *node_map, bitstr_t *core_map)
{
	uint32_t min_cpus,num_sum_cores_node,allocated_sum_cores_node,
		free_sum_threads_node ,num_cpus_per_node,allocated_nodes_count;
	int core_begin,core_end;
	int n;
	int i_first, i_last;
	...
	for( n=i_first ; n < i_last ; n++ )
	{
		if (!bit_test(node_map, n))
			continue;
		if(layouts_entity_get_kv (L_CONS_RES_PARTITION,node_record_table_ptr[n].name,L_ALLOCATED_NODE_COUNT,&allocated_nodes_count,L_T_UINT32) ==SLURM_SUCCESS)
		{
		}
...
clear_bit:	/* This node is not usable by this job */
		...
		core_begin = cr_get_coremap_offset (n);
		core_end = cr_get_coremap_offset (n);
		bit_clear_count_range (core_map,core_begin,core_end);
			 ...
			 int node_inx, uint16_t cr_type)
{
	...
	if (*avail_cpus > rem_cpus) {
		...
		*cpu_cnt = ((int)(*avail_cpus + vpus - 1) / vpus) * vpus;
		uint32_t cpu_cnt_tmp =(uint32_t ) *cpu_cnt ;
		//info("L-Before _cpus_to_use- cpu_cnt_tmp:%d *cpu_cnt:%d ",cpu_cnt_tmp,*cpu_cnt);
		if(layouts_entity_set_kv (L_CONS_RES_TOPOLOGY,node_record_table_ptr[node_inx].name,L_SELECTED_CPU_COUNT_IN_NODE,&cpu_cnt_tmp,L_T_UINT32) == SLURM_SUCCESS)
		{
			//layouts_entity_get_kv (L_CONS_RES_TOPOLOGY,node_record_table_ptr[node_inx].name,L_SELECTED_CPU_COUNT_IN_NODE,&cpu_cnt_tmp,L_T_UINT32);
			//info("L-_cpus_to_use- cpu_cnt_tmp:%d",cpu_cnt_tmp);
		}
	}
}
			...
			bool prefer_alloc_nodes)
{
	...
	if (switch_record_cnt && switch_record_table) 
	{
		...
		sprintf(str_i,"%s%d,%s%d",L_SELECTED_SUM_CPUS_IN_LEVEL,switch_record_table[root_switch_index].level
		        ,L_SELECTED_SUM_NODES_IN_LEVEL,switch_record_table[root_switch_index].level);
		if(layouts_entity_pull_kv(L_CONS_RES_TOPOLOGY,switch_record_table[root_switch_index].name,str_i) == SLURM_SUCCESS)
		{
		}
			...
			uint16_t *cpu_cnt, uint16_t cr_type)
{
	...
	uint32_t       *switches_cpu_cnt;		/* total CPUs on switch */
	uint32_t       *switches_node_cnt;		/* total nodes on switch */
	uint32_t       *switches_required;		/* set if has required node */
	...
	switches_cpu_cnt  = xmalloc(sizeof(uint32_t) * switch_record_cnt);
	switches_node_cnt = xmalloc(sizeof(uint32_t) * switch_record_cnt);
	switches_required = xmalloc(sizeof(uint32_t) * switch_record_cnt);
	...
	if (req_nodes_bitmap) 
	{
		bit_fmt(str_i,sizeof(str_i),req_nodes_bitmap);
		info("req_nodes_bitmap:%s",str_i);
		...
		for (i=first; ((i<=last) && (first>=0)); i++) 
		{
			...
			/*if(layouts_entity_set_kv (L_CONS_RES_TOPOLOGY,node_record_table_ptr[i].name,L_SELECTED_REQUIRED_IN_LEVEL,&avail_cpus,L_T_UINT32)== SLURM_SUCCESS)
	...
	//info("L-No specific required nodes, calculate CPU counts");
		...
		if(layouts_entity_pullget_kv (L_CONS_RES_TOPOLOGY,switch_record_table[root_switch_index].name,str_i,&switches_cpu_cnt[root_switch_index],L_T_UINT32) == SLURM_SUCCESS)
		{
			//info("L-switches_cpu_cnt[root_switch_index:%d]:%d",root_switch_index,switches_cpu_cnt[root_switch_index]);
		}

		/*sprintf(str_i,"%s%d",L_SELECTED_REQUIRED_IN_LEVEL,switch_record_table[root_switch_index].level);
		if(layouts_entity_pullget_kv (L_CONS_RES_TOPOLOGY,switch_record_table[root_switch_index].name,str_i,&switches_required[root_switch_index],L_T_UINT32) == SLURM_SUCCESS)
		{
			//info("L-switches_cpu_cnt[root_switch_index:%d]:%d",root_switch_index,switches_cpu_cnt[root_switch_index]);
		}
...
// Here we are getting cpu_count details in the level of all the switches.
			sprintf(str_i,"%s%d",L_SELECTED_SUM_CPUS_IN_LEVEL,switch_record_table[j].level);
			if(layouts_entity_get_kv (L_CONS_RES_TOPOLOGY,switch_record_table[j].name,str_i,&switches_cpu_cnt[j],L_T_UINT32) == SLURM_SUCCESS)
			{
	...
	/* Use required leaf switches first to minimize the total amount */
	/* of switches */
	...
	while ((max_nodes > 0) && ((rem_nodes > 0) || (rem_cpus > 0))) 
	{
		int *cpus_array = NULL, array_len;
		best_fit_cpus = best_fit_nodes = best_fit_sufficient = 0;
		/* select the best switch within the level-0(leaf) switches list */
		for (i=0 ; i < level_switch_count[0] && leaf_switch_count_used < level_switch_count[0] ; i++) 
		{
			...
			/*sprintf(str_i,"%s%d",L_SELECTED_REQUIRED_IN_LEVEL,switch_record_table[j].level);
			if(layouts_entity_get_kv (L_CONS_RES_TOPOLOGY,switch_record_table[j].name,str_i,&switches_required[j],L_T_UINT32) == SLURM_SUCCESS)
			{
			 ...
			 bool prefer_alloc_nodes,bool config)
{
	...
	char multi_key[100];
	sprintf(multi_key,"%s,%s",L_SELECTED_NODE_COUNT_IN_NODE,L_SELECTED_CPU_COUNT_IN_NODE);
	...
	if (switch_record_cnt && switch_record_table) 
	{
		...
		{			
			...
			{								
				multi_value[0] = 1; 
				multi_value[1] = (uint32_t) cpu_cnt[n];
				//info("Start of choose_node multi_entity_set");
				int processed = layouts_entity_set_mkv (L_CONS_RES_TOPOLOGY,node_record_table_ptr[n].name,multi_key,multi_value,length,L_T_UINT32);
				//info("processed :%d",processed );
			}
			else
			{				
				multi_value[0] = 0; 
		...
		/*size_t length = sizeof(uint32_t) * node_record_count;

		info("Start of choose_node multi_entity_set");
		int processed = layouts_multi_entity_set_sk_mv (L_CONS_RES_TOPOLOGY,node_name,L_SELECTED_NODE_COUNT_IN_NODE,node_count_value,length,L_T_UINT32);
		info("processed :%d",processed );
		processed = layouts_multi_entity_set_sk_mv (L_CONS_RES_TOPOLOGY,node_name,L_SELECTED_CPU_COUNT_IN_NODE,cpu_count_value,length,L_T_UINT32);
		info("processed :%d SLURM_SUCCESS:%d,SLURM_ERROR:%d",processed,SLURM_SUCCESS,SLURM_ERROR );*/
	}
			...
			bool qos_preemptor)
{
	...
	{
		if(MAX(min_nodes, req_nodes) > (num_sum_nodes - allocated_sum_nodes))
		{
			goto step2;
		}
	   ...
	   layouts_entity_get_kv (L_CONS_RES_PARTITION,L_CLUSTER,L_NUM_SUM_THREADS_CLUSTER,&num_sum_threads,L_T_UINT32)==SLURM_SUCCESS)
	{
		...
		if(MAX(number_of_tasks,job_ptr->details->min_cpus) > (num_sum_threads - allocated_sum_threads))
		{
			goto step2;
		}
	...
	clear_nodes_allocated (node_bitmap,free_cores);
