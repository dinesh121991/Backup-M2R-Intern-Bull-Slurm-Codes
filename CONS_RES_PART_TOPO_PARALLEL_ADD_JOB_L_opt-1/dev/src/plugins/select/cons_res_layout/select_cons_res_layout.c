...
void* _add_job_serial_multiset(void *job_rec)
{
	...
	int core_size = bit_fls(job->core_bitmap_original_job);
	i=bit_ffs(job ->core_bitmap_original_job);
	core_size = (core_size - i)+1;
	int str_size;
	...
	List socket_list = list_create(list_destroy);
