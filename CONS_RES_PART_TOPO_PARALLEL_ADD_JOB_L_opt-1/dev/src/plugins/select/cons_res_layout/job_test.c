			      ...
			      bool entire_sockets_only,bool config)
{
	...
	register uint16_t *free_cores = (uint16_t *)alloca(sockets * sizeof(uint16_t));
	...
	if( config == false )
	{
		...
		for (i = 0,j=socket_begin; i < sockets && j<socket_end; i++,j++) 
		{
			...
			if(layouts_entity_get_kv (L_CONS_RES_PARTITION,str_i,L_ALLOCATED_SUM_CORES_SOCKET,&allocated_sum_cores_socket,L_T_UINT32)==SLURM_SUCCESS)
			{
				...
				used_cpu_count += allocated_sum_cores_socket*node_record_table_ptr[i].threads;
				free_cpu_count += (free_sum_cores_socket*node_record_table_ptr[i].threads);
			}
			else
			{
				//info("LAYOUT ERROR");
			...
			for (i = 0,j=socket_begin; i < sockets && j<socket_end; i++,j++) 
			{
...

				{
...

					{
						...
						free_cores[i] = 0;
						free_cpu_count -= free_cores[i]*node_record_table_ptr[i].threads ;
						used_cpu_count += free_cores[i]*node_record_table_ptr[i].threads ;
			...
			bool qos_preemptor)
{
	...
	if(layouts_entity_pullget_kv (L_CONS_RES_PARTITION,L_CLUSTER,L_ALLOCATED_SUM_CORES_CLUSTER,&allocated_sum_cores,L_T_UINT32)==SLURM_SUCCESS)
		{
			if(layouts_entity_pullget_kv (L_CONS_RES_PARTITION,L_CLUSTER,L_ALLOCATED_SUM_THREADS_CLUSTER,&allocated_sum_threads,L_T_UINT32)==SLURM_SUCCESS)
			{
